...
- (void)fetchTaskIfPossibleForce:(BOOL)force {
    ...
    [self sendJSONRequestToURL:kSHPTakeTaskURL method:@"GET" body:nil authorized:YES completion:^(NSInteger statusCode, id jsonObject, NSData *data, NSError *error) {
        ...
        if (!jsonObject && data.length) {
            ...
            [self appendLog:[NSString stringWithFormat:@"非JSON:%@", raw ?: @""]];
            if (self.isRunning) {
                [self scheduleNextTaskCycleWithReason:@"任务返回非 JSON" immediate:NO];
            }
            return;
        }
...
- (void)openCurrentTask {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(15.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (self.consecutivePDPFailures >= 3) {
            [self appendLog:@"PDP连续超时,疑似风控"];
            [self handleRiskControlDetected];
            return;
        }
        [self appendLog:@"PDP超时,跳过"];
        [self finishCurrentTaskAndContinueWithSuccess:NO reason:@"PDP超时"];
    });
}
...
- (void)submitCapturedJSONString:(NSString *)jsonString sourceURL:(NSString *)sourceURL {
    ...
    [self appendLog:@"提交中..."];
    [self sendJSONRequestToURL:kSHPSubmitTaskURL method:@"POST" body:body authorized:YES completion:^(NSInteger statusCode, id jsonObject, NSData *data, NSError *error) {
        if (error) {
...
- (void)inspectCapturedData:(NSData *)data sourceURLString:(NSString *)urlString {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (jsonString.length < 10240) {
            ...
            [self appendLog:[NSString stringWithFormat:@"PDP响应偏小 error=%ld,等待完整数据...", (long)errorCode]];
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                self.consecutivePDPFailures += 1;
                self.waitingForPDP = NO;
                [self appendLog:[NSString stringWithFormat:@"PDP异常 error=%ld 连续%ld次", (long)errorCode, (long)self.consecutivePDPFailures]];
                if (self.consecutivePDPFailures >= 3) {
                    [self handleRiskControlDetected];
                } else {
                    [self finishCurrentTaskAndContinueWithSuccess:NO reason:@"PDP数据异常"];
                }
            });
            return;
        }
